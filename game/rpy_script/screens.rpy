...
screen quick_menu():
    ...
    if quick_menu:
        ...
        hbox:
            ...
            textbutton ("Q.Save") action QuickSave()
            textbutton ("Q.Load") action QuickLoad()
...
screen main_menu():
    ...
    vbox:
        ...
        hbox:
            spacing 120  # 按钮横向间距
            # 新游戏按钮
            textbutton _("START CHAT"):
                action Start()
                text_size 45
                text_color "#FFFFFF"      # 默认白色
                text_hover_color "#F4D03F" # 悬停金色
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    style_prefix "chat"
                    label _("对话设置")
                    textbutton _("Chat_Character") action ShowMenu("parm_choice", "chat_character", 'chat_characters_dict')
                    textbutton _("TTS_Model") action ShowMenu("parm_choice", "tts_model", 'tts_model_list')
                    textbutton _("TTS_Voice") action ShowMenu("parm_choice", "tts_voice", 'tts_voice_list')

                vbox:
                    style_prefix "api"
                    label _("API接口设置")
                    textbutton _("LLM_api_base_url") action ShowMenu("parm_edit", "llm_api_base_url")
                    textbutton _("LLM_api_key") action ShowMenu("parm_edit", "llm_api_key")
                    textbutton _("TTS_api_base_url") action ShowMenu("parm_edit", "tts_api_base_url")
                    textbutton _("TTS_api_key") action ShowMenu("parm_edit", "tts_api_key")
...
init python:
    ...
    def refresh_list(parm):
        if parm == "chat_character":
            chat_characters_file = open(str(config.basedir)+ f"\\game\\characters\\characters.json", "r", encoding="utf-8")
            chat_characters_dict = chat_characters_file.read()
            chat_characters_dict = json.loads(chat_characters_dict)
            chat_characters_file.close()
            renpy.run(SetField(preferences, "chat_characters_dict", chat_characters_dict))
        elif parm == "tts_model":
            tts_model_list = []
            response = renpy.fetch(f"{preferences.tts_api_base_url}/v1/audio/models", method="GET", result="json")
            for item in response["models"]:
                tts_model_list.append(item["id"])
            renpy.run(SetField(preferences, "tts_model_list", tts_model_list))
        elif parm == "tts_voice":
            tts_voice_list = []
            response = renpy.fetch(f"{preferences.tts_api_base_url}/v1/audio/voices", method="GET", result="json")
            for item in response["voices"]:
                tts_voice_list.append(item["name"])
            renpy.run(SetField(preferences, "tts_voice_list", tts_voice_list))
...
style chat_label is pref_label
style chat_label_text is pref_label_text
style chat_button is gui_button
style chat_button_text is gui_button_text
style chat_vbox is pref_vbox

style api_label is pref_label
style api_label_text is pref_label_text
style api_button is gui_button
style api_button_text is gui_button_text
style api_vbox is pref_vbox
...
# parm_edit 屏幕
###############################################################################
screen parm_edit(parm, title_name='parm_edit', from_game=False):

    ## 显示此屏幕时，确保其他屏幕无法输入。
    modal True
    zorder 100

    default temp_value = getattr(preferences, parm, "")
    ...
    style_prefix "parm_edit"

    frame at _fade_in_out:
        xalign 0.5
        yalign 0.5
        vbox:
            spacing 30
            ...
            text title_name:
                xalign 0.5
                yalign 0.0
                style "parm_edit_title"

            # 输入框
            input:
                id "input"
                value ScreenVariableInputValue("temp_value")
                copypaste True
            ...
            hbox:
                ...
                textbutton _("确定"):
                    if not from_game:
                        action [
                            SetField(preferences, parm, temp_value),  # 设置参数值
                            Hide("parm_edit")  # 关闭弹窗
                        ]
                    else:
                        action [
                ...
                if not from_game:
                    textbutton _("取消"):
                        action Hide("parm_edit")
                else:
                    textbutton _("返回"):
                        action Return()

transform _fade_in_out:
    on show:
        alpha 0.0
        yoffset 20
    ...
    on hide:
        alpha 1.0
        yoffset 0
        easeout 0.25 yoffset 20 alpha 0.0

style parm_edit_frame is gui_frame
style parm_edit_prompt is gui_prompt
style parm_edit_prompt_text is gui_prompt_text
style parm_edit_button is gui_medium_button
style parm_edit_button_text is gui_medium_button_text

style parm_edit_frame:
    background Frame([ "gui/confirm_frame.png", "gui/frame.png"], gui.confirm_frame_borders, tile=gui.frame_tile)
    padding gui.confirm_frame_borders.padding
    xalign .5
    yalign .5

style parm_edit_prompt_text:
    textalign 0.5
    layout "subtitle"

style parm_edit_button:
    properties gui.button_properties("confirm_button")

style parm_edit_button_text:
    properties gui.text_properties("confirm_button")
...
# parm_choice 屏幕
###############################################################################
screen parm_choice(parm, items_name, title_name='parm_choice', from_game=False):
    modal True
    zorder 100
    default selected_value = getattr(preferences, parm, None)
    default items = getattr(preferences, items_name, None)    # 获取当前参数值
    ...
    # 选择窗口主体
    frame at _slide_in_out:
        style_prefix "parm_choice"
        xalign 0.5
        yalign 0.5
        vbox:
            spacing 40
            ...
            vbox:
                ...
                for item in items:
                    textbutton str(item):
                        action SetScreenVariable("selected_value", item)
                        text_style "parm_choice_text"
                        if selected_value == item:
                            text_selected_color "#FFD700"     # 黄金色标记选中项
            ...
            hbox:
                ...
                textbutton _("刷新列表"):
                    if not from_game:
                        action [
                            Function(refresh_list, parm=parm),  # 刷新列表
                            Hide("parm_choice"),
                            ShowMenu("parm_choice", parm, items_name)  # 重新显示菜单
                        ]
                    else:
                        action [
                            Function(refresh_list, parm=parm),  # 刷新列表
                ...
                textbutton _("确定"):
                    if not from_game:
                        action [
                            SetField(preferences, parm, selected_value),
                            Hide("parm_choice")
                        ]
                    else:
                        action [
                ...
                if not from_game:
                    textbutton _("取消"):
                        action Hide("parm_choice")
                else:
                    textbutton _("返回"):
                        action Return(value='return')

transform _slide_in_out:
    on show:
        alpha 0.0
        yoffset 50
    ...
    on hide:
        alpha 1.0
        easeout 0.3 yoffset 20 alpha 0.0

style parm_choice_text:
    size 28
    color "#CCCCCC"
    hover_color "#FFFFFF"
...
style parm_choice_vbox is vbox
style parm_choice_button is button
style parm_choice_button_text is button_text

style parm_choice_vbox:
    spacing gui.choice_spacing

style parm_choice_button is default:
    properties gui.button_properties("choice_button")

style parm_choice_button_text is default:
    properties gui.text_properties("choice_button")
...
screen loading_pray():
    ...
    vbox at pray_effect:
        ...
        text "少女祈祷中...":
            outlines [ (3, "#0008", 0, 0) ]  # 黑色描边

transform pray_effect:
    on show:
        alpha 0.0 xoffset 50
        parallel:
            easein 0.5 alpha 1.0
        parallel:
            easein_quad 0.5 xoffset 0
    ...
    on hide:
        ...
        parallel:
            easeout_quad 0.3 xoffset 30

style pray_style_text:
    size 42
    color "#f8e5e5"  # 浅樱色
    kerning 2
    bold True
...
screen gallery:
    ...
    # 用于响应后返回主菜单的界面。
    # 也能用于导航到其他画廊界面。
    textbutton "Return" action Return() xalign 0.8 yalign 1.0
